...
def cmd_parse(args: argparse.Namespace) -> None:
    ...
    non_html_docs = grouped.get("mineru_vlm", [])
    if non_html_docs:
        ...
        # run_mineru_batches is idempotent: successful upload parts are reused;
        # only missing/failed parts are submitted again.
        token = get_token()
        all_records.extend(run_mineru_batches(non_html_docs, output_dir / "mineru_vlm", token=token, options=options))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parse.add_argument("--extra-format", action="append", choices=["docx", "html", "latex"], help="额外导出格式，可重复传入")
    parse.add_argument("--poll-interval", type=int, default=10)
    parse.add_argument("--max-wait", type=int, default=3600)
    parse.add_argument("--batch-size", type=int, default=50)
    parse.add_argument("--download-retries", type=int, default=5)
    parse.add_argument("--download-chunk-size-mb", type=int, default=1)
    ...
    parse.add_argument("--jina-request-gap", type=float, default=0.2, help="连续 Jina 请求之间的轻量等待秒数")
    parse.add_argument("--chunk-chars", type=int, default=1800)
    parse.add_argument("--chunk-overlap", type=int, default=180)
