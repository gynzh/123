...
graph TD
  ...
  E --> F
  F --> G["监管链"]
    H["业务链A"] <--> I["业务链A的DID解析器"]
  I --> J["业务链A"]
    K["业务链B"] <--> L["业务链B的DID解析器"]
  L --> M["业务链B"]
    N["业务链C"] <--> O["业务链C的DID解析器"]
  O --> P["业务链C"]
```
</details>
...
graph TD
  ...
  B -->|检查本地是否存在?| C["监管链DID解析器"]
  C -->|本地不存在则向监管链DID解析器转发解析请求| D["目标业务链DID解析器"]
  D -->|向目标链解析器转发解析请求| E["目标业务链DID解析器"]
  E -->|返回远程DID文档| F["目标业务链DID解析器"]
  F -->|转发远程DID文档| G["本地业务链DID解析器"]
  G -->|依据解析情况返回本地或远程DID文档| H["目标业务链DID解析器"]
```
</details>
...
a) 个人和机构实体DID创建过程应确保身份的真实可信，其中身份注册和身份核实环节应符合JR/T0184中13.3和13.4的要求；  
...
graph TD
  ...
  B -->|K 请求 VP 持有者的DID文档| C["DID系统"]
  B -->|K 返回查询结果| D["验证 VP 的证明属性"]
  D -->|K 验证失败返回错误| E["验证者"]
  E -->|K 返回验证结果| F["验证者"]
  F -->|K 发送认证挑战值| G["构建VP"]
  G -->|K 出示 VP| B
...
![](images/7c7ce9b2a82bcf0f9dfb9de5ec06f48727ce3447ea400d97fd6a689c576eb795.jpg)
...
graph TD
  ...
  C -->|本地存储VC| D["数据使用方"]
  D -->|查询相关DID| E["DID系统"]
  E -->|返回查询结果| F["依据9.6条的规定验证VP"]
  F -->|返回校验结果及使用收据| G["返回校验结果及使用收据"]
  G -->|提示包含VC的VP| D
```
</details>
...
graph TD
  ...
  B -->|返回代理存储收据| D["校验失败"]
  D -->|返回错误| E["校验授权VC"]
  E -->|校验失败| F["提示: 出示包含授权VC和数据VC的VP"]
  F --> G["数据使用方"]
  G -->|查询相关DID| H["DID系统"]
  H -->|返回查询结果| I["依据9.6条规定验证VP"]
...
c) 数据使用方应按照 9.6 中的要求验证 VP 的正确性。当采用授权 VC 的方式时，数据使用方应按照 9.5 中的要求验证授权 VC 的正确性；  
...
市场主体已完成 DID 注册。市场主体为了办理新的业务（如：专精特新企业申请），则需要授权给区域性股权市场从征信机构获取企业相关的数据，以便可以更好地评估企业状况。
...
"proof": {
    ...
    "proofPurpose":"assertionMethod",
    "proofValue":
    "eyJhbGciOiJSUzI1NiIsImI2NCI6ZmFsc2UsImNyaXQiOlsiYjYOIl19..TCYt5XsITJX1CxPCT8yAV-TVkIEq_PbChOMqsLfRoPsnsgw5WEutsOlmq-pQy7UJiN5mgRxD-WUcX16dUEMGlv50aqzpqh4Qktb3rk-BuQy72IFLOqVOG_zS245-kronKb78cPN25DGlcTwLtjPAYuNzVBAh4vGHSrQyHUdBBPM"
}
```
...
X16dUEMGlv50aqzpqh4Qktb3rk-BuQy72IFLOqVOG_zS245-kronKb78cPN25DGlcTwLtj
